...
public class StoreController  implements BaseController {
    ...
    public ArrayList<Store<Item>> index() {
    ...
    public Store<Item> getCheapestItems(){
        Store<Item> storeWithCheapestItem = null;
        BigDecimal cheapestPrice = BigDecimal.valueOf(Double.MAX_VALUE);
        for (Store<Item> store : Data.getInstance().storeList) {
